...
void RGBDisplay::init() {
	...
	if (RGB_DEBUGGING) {Serial.println("RGBDisplay set up complete.");}
}
...
void RGBDisplay::update() {
	...
	if (action == FLASH) {
		...
		if (elapsed_time < 0) { // Then the millis has rolled back to 0.
			...
			if (RGB_DEBUGGING) {Serial.println("Millis has rolled to 0 - resetting flash start.");}
		...
		{ 
			if (RGB_DEBUGGING) {Serial.println("RGB LED Flash On");}
		...
		{ 
			if (RGB_DEBUGGING) {Serial.println("RGB LED Flash Off");}
...
void RGBDisplay::solid(bool r, bool g, bool b) {
	if (r || g || b) { // Need at least one to be on before solid
		if (RGB_DEBUGGING) {Serial.println("RGB LED solid initialized.");}
...
void RGBDisplay::flash(bool r_on, bool g_on, bool b_on, unsigned int num_flashes=0, unsigned int on_len=1000, unsigned int off_len=1000, bool r_off=0, bool g_off=0, bool b_off=0) {
	if (RGB_DEBUGGING) {Serial.println("RGB LED flash initialized.");}
	
	// Don't start another flash until the current one is cancelled.
	if (action == FLASH) {
		return;
	}
...
void RGBDisplay::off() {
	if (RGB_DEBUGGING) {Serial.println("RGB LED turned off.");}
